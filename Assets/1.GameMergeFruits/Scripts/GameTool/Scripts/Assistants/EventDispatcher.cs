...
namespace My.Tool
{
    public static class EventDispatcher
    {
        ...
        public static void PostEvent(EventID eventID, Component sender, object param = null)
        {
            ...
            {
                //Debug.Log("EventDispatcher check: " + actionList.Count);
                for (int i = 0, amount = actionList.Count; i < amount; i++)
                {
                    try
                    {
                        actionList[i](sender, param);
                        //Debug.Log("EventDispatcher check i: " + i + " / " + actionList.Count);
                    }
                    catch (Exception e)
                    {
...
public enum EventID
{
    Gameplay_FinishTutorial,
    Gameplay_FinishDraw,
    Gameplay_LineStop,
    Gameplay_ObjectStop,

    Gameplay_ReverseLadder,

    Gameplay_ActiveObject,
    Gameplay_StartGame,
    Gameplay_Win,
    Gameplay_Lose,
    Gameplay_NextLevel,
